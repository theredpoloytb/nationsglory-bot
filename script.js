...
async function loadCS(){const s=$('ck-srv').value;if(!s)return;$('ck-suggest').innerHTML=`<span style="font-family:var(--M);font-size:.5rem;color:var(--t3)">Chargement...</span>`;try{if(!cc[s]){const d=await api('/api/countries/'+s);const raw1=d.claimed||d.countries||[];cc[s]=raw1.map(x=>x.name||x).filter(Boolean).sort((a,b)=>a.toLowerCase().localeCompare(b.toLowerCase()));}rCS(s);}catch{cc[s]=[];$('ck-suggest').innerHTML='';}}
...
async function cwLoadCountries(){
  ...
  try{
    if(!cwCountries[s]){const d=await api('/api/countries/'+s);const raw3=d.claimed||d.countries||[];cwCountries[s]=raw3.map(x=>x.name||x).filter(Boolean).sort((a,b)=>a.localeCompare(b));}
    cwFilterCountries();
...
async function refLoadCountries(){
  ...
  try{
    if(!refCountryCache[s]){const d=await api('/api/countries/'+s);const raw4=d.claimed||d.countries||[];refCountryCache[s]=raw4.map(x=>x.name||x).filter(Boolean).sort((a,b)=>a.localeCompare(b));}
    refFilterCountries();
