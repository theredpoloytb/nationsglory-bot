...
async function loadCS(){const s=$('ck-srv').value;if(!s)return;$('ck-suggest').innerHTML=`<span style="font-family:var(--M);font-size:.5rem;color:var(--t3)">Chargement...</span>`;try{if(!cc[s]){const r=await fetch(`https://publicapi.nationsglory.fr/country/list/${s}`,{headers:{'Authorization':'Bearer NGAPI_q05@rd^9Gg!@A9(4YYQEHVj9)6fNTGF2c02f64647e5f99a75001c7cb30c1e8e5','accept':'application/json'}});const d=await r.json();const raw=d.claimed||[];cc[s]=raw.map(x=>x.name||x).filter(Boolean).sort((a,b)=>a.toLowerCase().localeCompare(b.toLowerCase()));}rCS(s);}catch(e){cc[s]=[];$('ck-suggest').innerHTML=`<span style="color:var(--red)">Erreur</span>`;}}
...
async function cwLoadCountries(){
  ...
  try{
    if(!cwCountries[s]){const r=await fetch(`https://publicapi.nationsglory.fr/country/list/${s}`,{headers:{'Authorization':'Bearer NGAPI_q05@rd^9Gg!@A9(4YYQEHVj9)6fNTGF2c02f64647e5f99a75001c7cb30c1e8e5','accept':'application/json'}});const d=await r.json();const raw3=d.claimed||[];cwCountries[s]=raw3.map(x=>x.name||x).filter(Boolean).sort((a,b)=>a.localeCompare(b));}
    cwFilterCountries();
...
async function refLoadCountries(){
  ...
  try{
    if(!refCountryCache[s]){const r=await fetch(`https://publicapi.nationsglory.fr/country/list/${s}`,{headers:{'Authorization':'Bearer NGAPI_q05@rd^9Gg!@A9(4YYQEHVj9)6fNTGF2c02f64647e5f99a75001c7cb30c1e8e5','accept':'application/json'}});const d=await r.json();const raw4=d.claimed||[];refCountryCache[s]=raw4.map(x=>x.name||x).filter(Boolean).sort((a,b)=>a.localeCompare(b));}
    refFilterCountries();
