...
async function loadDash(){
  ...
  try{
    ...
    const getCount=s=>{const d=(all[s]||[]).length;return d>0?d:(pc[s]?.players??0);};
    const dynmapDown=s=>(all[s]||[]).length===0&&(pc[s]?.players??0)>0;
    ...
    if(cards.length===SRV.length){SRV.forEach((s,i)=>{const cnt=getCount(s),c=cards[i],n=c.querySelector('.sc-n'),b=c.querySelector('.sbar-f'),lbl=c.querySelector('.sc-lbl');if(n&&parseInt(n.textContent)!==cnt){n.style.opacity='.1';setTimeout(()=>{n.textContent=cnt;n.style.opacity='1';},120);}if(b)b.style.width=Math.round(cnt/mx*100)+'%';if(lbl)lbl.textContent=dynmapDown(s)?'📡 PLAYERCOUNT':'EN LIGNE';});}
