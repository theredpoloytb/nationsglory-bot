...
async function loadKP(){try{const d=await fetch(API+'/api/known_players',{headers:{..._authHeader()}}).then(r=>r.json());oP=[...new Set([...(d.players||[]),...oP])].sort((a,b)=>a.toLowerCase().localeCompare(b.toLowerCase()));rAT('ca-pl','ppCA');rAT('st-pl','ppST');rAT('wl-pl','ppWL');rAT('sword-pl','ppSword');}catch{}}
